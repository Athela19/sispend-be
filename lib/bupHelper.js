 ...
 * @returns {Promise<string>} Status BUP
 */
...
// helper tgl bup
function getTanggalBup(ttl, bupAge) {
  const birth = new Date(ttl);
  return new Date(
    birth.getFullYear() + bupAge,
    birth.getMonth(),
    birth.getDate()
  );
}
...
export async function checkBupStatus(personil) {
  if (!personil?.TTL || !personil?.PANGKAT) return "Unknown";

  const rank = getRankFromPangkat(personil.PANGKAT);

  const bupAge =
    ["brigjen", "mayjen", "letjen"].includes(rank)
      ? await getBupForRank(rank)
      : DEFAULT_BUP_AGE;

  if (!bupAge) return "Unknown";

  const today = new Date();
  const tanggalBup = getTanggalBup(personil.TTL, bupAge);

  if (today >= tanggalBup) {
    return "Mencapai BUP";
  }

  const diff = diffDate(today, tanggalBup);

  if (diff.years > 0) {
    return `${diff.years} Tahun ${diff.months} Bulan Lagi`;
  }
